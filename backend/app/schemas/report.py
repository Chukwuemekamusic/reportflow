...
class ReportJobFilters(BaseModel):
    date_from: datetime | None = Field(
        None, examples=["2025-01-01"], description="ISO 8601 date string"
    )
...
class ReportJobCreate(BaseModel):
    ...
    priority: int = Field(
        default=5, ge=1, le=9, description="1=highest, 5=normal, 9=lowest"
    )
    ...
    filters: dict[str, Any] | None = Field(
        None, description="Optional report-specific filters"
    )
...
def job_to_response(job: ReportJob, base_url: str = "") -> ReportJobResponse:
    ...
    return ReportJobResponse(
        ...
        links=JobLinks(
            ...
            download=f"{base_url}/api/v1/reports/{job.id}/download"
