...
async def test_update_queue_metrics_reads_all_celery_queues() -> None:
    ...
    with (
        patch.object(main, "QUEUE_DEPTH", gauge),
        patch.object(
    ...
    assert fake_redis.llen.await_args_list == [
        call("high"),
        call("default"),
        call("low"),
    ]
...
async def test_lifespan_starts_and_stops_queue_metrics_task() -> None:
    ...
    with (
        patch.object(main, "ensure_bucket_exists", AsyncMock()) as mock_bucket,
        patch.object(main, "update_queue_metrics", AsyncMock()) as mock_update,
        patch.object(main, "refresh_queue_metrics", fake_refresh_queue_metrics),
    ):
...
def test_create_app_excludes_metrics_endpoint_from_http_metrics() -> None:
    ...
    with patch.object(
        main, "Instrumentator", return_value=instrumentator_instance
    ) as mock_instrumentator:
        app = main.create_app()
