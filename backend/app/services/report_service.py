...
async def create_report_job(
    ...
    if payload.idempotency_key:
        cache_key = (
            f"idempotency:{str(current_user.tenant_id)}:{payload.idempotency_key}"
        )
    ...
    if payload.idempotency_key:
        ...
        if existing:
            ...
            _cache_idempotency_key(
                current_user.tenant_id, payload.idempotency_key, existing.id
            )
    ...
    if payload.idempotency_key:
        _cache_idempotency_key(current_user.tenant_id, payload.idempotency_key, job.id)
...
async def cancel_job(
    ...
    if job.status in ("completed", "failed", "cancelled"):
        logger.warning(
            f"[{job_id}] Cannot cancel job — status is already '{job.status}'"
        )
        return job
...
def _cache_idempotency_key(tenant_id: str, idempotency_key: str, job_id: str) -> None:
    ...
    try:
        ...
        logger.warning(
            f"Failed to write idempotency cache for key {idempotency_key}: {e}"
        )
