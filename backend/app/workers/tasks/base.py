...
_pubsub_client = redis_sync.Redis.from_url(
...
class ReportBaseTask(Task):
    ...
    # progress reporting
    def update_progress(
        self, job_id: str, progress: int, stage: str, eta_secs: int = 0
    ):
        """
            ...
            await session.execute(
                ...
                .values(
                    ...
                    started_at=func.coalesce(
                        ReportJob.started_at, datetime.now(timezone.utc)
                    ),
                )
            )
    ...
    def on_success(self, retval, task_id, args, kwargs):
        ...
        try:
            ...
            logger.error(
                f"[{job_id}] Failed to update job status on success: {e}", exc_info=True
            )
    ...
    async def _async_on_success(self, job_id: str, file_key: str):
        ...
        if not user_id:
            logger.error(
                f"[{job_id}] Could not find user_id to decrement rate-limit counter"
            )
            return
    ...
    async def _async_on_failure(
        self, job_id: str, error_message: str, error_trace: str
    ):
        from app.db.base import AsyncSessionLocal
        from app.db.models.report_job import ReportJob
